...
export default function Login() {
  ...
  async function entrar() {
    ...
    try {
      const response = await fetch(
        'https://qfesxpcuhsrfdohnsleg.supabase.co/auth/v1/token?grant_type=password',
        {
          method: 'POST',
          headers: {
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', minHeight: '100vh', background: '#f5f5f0', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#fff', borderRadius: '12px', border: '0.5px solid #e0e0d8', padding: '2rem', width: '100%', maxWidth: '380px', margin: '1rem' }}>
        <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
          <div style={{ width: '48px', height: '48px', background: '#085041', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1rem', fontSize: '22px', color: '#E1F5EE', fontWeight: '700' }}>P</div>
          <div style={{ fontSize: '18px', fontWeight: '600', color: '#085041' }}>Pozeros Agro</div>
          <div style={{ fontSize: '13px', color: '#888', marginTop: '4px' }}>Panel de administración</div>
        </div>

        {error && (
          <div style={{ background: '#FCEBEB', border: '0.5px solid #F09595', borderRadius: '8px', padding: '10px 14px', fontSize: '13px', color: '#A32D2D', marginBottom: '1rem' }}>
            {error}
          </div>
        )}
        ...
        <button onClick={entrar} disabled={cargando}
          style={{ width: '100%', padding: '11px', background: '#085041', color: '#E1F5EE', border: 'none', borderRadius: '8px', fontSize: '15px', cursor: 'pointer', fontWeight: '500' }}>
