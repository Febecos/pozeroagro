...
export default function Admin() {
  ...
  async function actualizarCampo(id, campo, valor) {
    await supabase.from('perforistas').update({ [campo]: valor }).eq('id', id)
    setPerforistas(prev =>
  ...
  async function cambiarEstado(id, estado) {
    ...
    await supabase.from('perforistas').update({ estado, ...extras }).eq('id', id)
    setPerforistas(prev =>
      prev.map(p => p.id === id ? { ...p, estado, ...extras } : p)
    )
  }
  ...
  function Toggle({ id, campo, valor }) {
    return (
      <div
        onClick={() => actualizarCampo(id, campo, !valor)}
        style={{
          width: '36px', height: '20px', borderRadius: '10px',
          background: valor ? '#1D9E75' : '#ccc',
      ...
      >
        <div style={{
          ...
          left: valor ? '18px' : '2px', transition: 'left .2s'
        }} />
      </div>
    )
  }
  ...
  const filtrados = perforistas.filter(p => {
    ...
    const coincideBusqueda = !busqueda ||
      `${p.nombre} ${p.apellido} ${p.localidad} ${p.provincia}`.toLowerCase().includes(busqueda.toLowerCase())
    ...
    <div style={{ fontFamily: 'sans-serif', minHeight: '100vh', background: '#f5f5f0' }}>
      ...
      <div style={{ padding: '1.25rem 1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '1rem', flexWrap: 'wrap' }}>
          <input
            type="text" placeholder="Buscar perforista..."
          ...
          {['todos','pendiente','activo','cliente','inactivo'].map(f => (
            <button key={f} onClick={() => setFiltro(f)}
              style={{
                ...
                border: filtro === f ? '1px solid #1D9E75' : '0.5px solid #ccc',
                background: filtro === f ? '#E1F5EE' : '#fff',
                color: filtro === f ? '#085041' : '#666',
        ...
        <div style={{ background: '#fff', borderRadius: '12px', border: '0.5px solid #e0e0d8', overflow: 'hidden' }}>
          ...
          {!cargando && filtrados.length === 0 && (
            <div style={{ padding: '2rem', textAlign: 'center', color: '#888' }}>No hay perforistas para mostrar.</div>
          )}

          {filtrados.map((p, i) => (
            <div key={p.id} style={{
              display: 'grid',
              gridTemplateColumns: '1.5fr 1fr 80px 70px 70px 70px 70px 70px 100px',
              padding: '.65rem 1rem', alignItems: 'center',
              borderBottom: i < filtrados.length - 1 ? '0.5px solid #f0f0e8' : 'none',
              fontSize: '13px'
            }}>
              ...
              <div>
                <select
                  ...
                  style={{
                    ...
                    background: badgeColor[p.estado]?.bg || '#f0f0f0',
                    color: badgeColor[p.estado]?.color || '#666',
                    fontWeight: '500'
                  }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'center' }}>
                <Toggle id={p.id} campo="visible_telefono" valor={p.visible_telefono} />
              </div>
              <div style={{ display: 'flex', justifyContent: 'center' }}>
                <Toggle id={p.id} campo="visible_whatsapp" valor={p.visible_whatsapp} />
              </div>
              <div style={{ display: 'flex', justifyContent: 'center' }}>
                <Toggle id={p.id} campo="visible_instagram" valor={p.visible_instagram} />
              </div>
              <div style={{ display: 'flex', justifyContent: 'center' }}>
                <Toggle id={p.id} campo="visible_facebook" valor={p.visible_facebook} />
              </div>
              <div style={{ display: 'flex', justifyContent: 'center' }}>
                <Toggle id={p.id} campo="visible_email" valor={p.visible_email} />
              </div>

              {/* Profundidad */}
              <div style={{ textAlign: 'center', color: '#666' }}>
                {p.profundidad_max ? `${p.profundidad_max}m` : '—'}
              </div>
              ...
              <div style={{ display: 'flex', gap: '4px' }}>
                {p.telefono && (
                  <a href={`tel:${p.telefono}`}
                    style={{ fontSize: '11px', padding: '3px 8px', borderRadius: '4px', background: '#E1F5EE', color: '#085041', textDecoration: 'none' }}>
                    📞
                  </a>
                )}
                {p.whatsapp && (
                  <a href={`https://wa.me/${p.whatsapp.replace(/\D/g,'')}`} target="_blank" rel="noreferrer"
                    style={{ fontSize: '11px', padding: '3px 8px', borderRadius: '4px', background: '#EAF3DE', color: '#27500A', textDecoration: 'none' }}>
                    WA
                  </a>
                )}
              </div>
        ...
        <div style={{ fontSize: '12px', color: '#aaa', textAlign: 'center', marginTop: '1rem' }}>
          {filtrados.length} perforistas · Al cambiar estado a "Cliente" el WhatsApp se activa automáticamente
        </div>
